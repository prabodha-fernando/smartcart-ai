...
export const getCart = asyncHandler(async (req, res) => {
  const cart = await getCartForUser(getUserId((req as any).userId));
...
export const removeItem = asyncHandler(async (req, res) => {
  const { id } = req.params as CartItemParamInput;
  const cart = await removeCartItem(getUserId((req as any).userId), id);
...
export const clearCart = asyncHandler(async (req, res) => {
  const cart = await clearUserCart(getUserId((req as any).userId));
