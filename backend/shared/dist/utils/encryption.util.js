        ...
        worker.on('message', (msg) => {
            ...
            if (p) {
                ...
                if (msg.error)
                    ...
                    p.resolve(msg.result || msg.status);
            }
        });
        worker.on('error', (err) => {
 ...
 */
export async function encryptPayloadAsync(payload) {
 ...
 */
export async function decryptPayloadAsync(ivHex, encryptedHex) {
    return new Promise((resolve, reject) => {
        const id = ++messageId;
        pendingPromises.set(id, { resolve, reject });
...
export function decryptPayload(ivHex, encryptedHex) {
